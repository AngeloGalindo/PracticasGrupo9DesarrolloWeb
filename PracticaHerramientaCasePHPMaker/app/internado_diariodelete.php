...
$internado_diario_delete = NULL; // Initialize page object first

class cinternado_diario_delete extends cinternado_diario {
	...
	// Page object name
	var $PageObjName = 'internado_diario_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("internado_diariolist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in internado_diario class, internado_diarioinfo.php
	...
	function DeleteRows() {
		...
		if ($DeleteRows) {
			...
			foreach ($rsold as $row) {
				...
				$sThisKey .= $row['idinternado_diario'];
				$conn->raiseErrorFn = 'ew_ErrorFn';
...
// Create page object
if (!isset($internado_diario_delete)) $internado_diario_delete = new cinternado_diario_delete();

// Page init
$internado_diario_delete->Page_Init();

// Page main
$internado_diario_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$internado_diario_delete->Page_Render();
?>
<?php include_once $EW_RELATIVE_PATH . "header.php" ?>
<script type="text/javascript">

// Page object
var internado_diario_delete = new ew_Page("internado_diario_delete");
internado_diario_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = internado_diario_delete.PageID; // For backward compatibility

// Form object
var finternado_diariodelete = new ew_Form("finternado_diariodelete");

// Form_CustomValidate event
finternado_diariodelete.Form_CustomValidate = 
...
finternado_diariodelete.ValidateRequired = true;
<?php } else { ?>
finternado_diariodelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
finternado_diariodelete.Lists["x_idinternado"] = {"LinkField":"x_idinternado","Ajax":true,"AutoFill":false,"DisplayFields":["x_idinternado","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
finternado_diariodelete.Lists["x_idcuenta"] = {"LinkField":"x_idcuenta","Ajax":true,"AutoFill":false,"DisplayFields":["x_idcuenta","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
if ($internado_diario_delete->Recordset = $internado_diario_delete->LoadRecordset())
	$internado_diario_deleteTotalRecs = $internado_diario_delete->Recordset->RecordCount(); // Get record count
if ($internado_diario_deleteTotalRecs <= 0) { // No record found, exit
	if ($internado_diario_delete->Recordset)
		$internado_diario_delete->Recordset->Close();
	$internado_diario_delete->Page_Terminate("internado_diariolist.php"); // Return to list
}
?>
<div class="ewToolbar">
...
<?php $internado_diario_delete->ShowPageHeader(); ?>
<?php
$internado_diario_delete->ShowMessage();
?>
<form name="finternado_diariodelete" id="finternado_diariodelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($internado_diario_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $internado_diario_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="internado_diario">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($internado_diario_delete->RecKeys as $key) { ?>
...
<?php if ($internado_diario->idinternado_diario->Visible) { // idinternado_diario ?>
		<th><span id="elh_internado_diario_idinternado_diario" class="internado_diario_idinternado_diario"><?php echo $internado_diario->idinternado_diario->FldCaption() ?></span></th>
<?php } ?>
<?php if ($internado_diario->idinternado->Visible) { // idinternado ?>
		<th><span id="elh_internado_diario_idinternado" class="internado_diario_idinternado"><?php echo $internado_diario->idinternado->FldCaption() ?></span></th>
<?php } ?>
<?php if ($internado_diario->idcuenta->Visible) { // idcuenta ?>
		<th><span id="elh_internado_diario_idcuenta" class="internado_diario_idcuenta"><?php echo $internado_diario->idcuenta->FldCaption() ?></span></th>
<?php } ?>
<?php if ($internado_diario->fecha->Visible) { // fecha ?>
		<th><span id="elh_internado_diario_fecha" class="internado_diario_fecha"><?php echo $internado_diario->fecha->FldCaption() ?></span></th>
<?php } ?>
<?php if ($internado_diario->costo->Visible) { // costo ?>
		<th><span id="elh_internado_diario_costo" class="internado_diario_costo"><?php echo $internado_diario->costo->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$internado_diario_delete->RecCnt = 0;
$i = 0;
while (!$internado_diario_delete->Recordset->EOF) {
	$internado_diario_delete->RecCnt++;
	$internado_diario_delete->RowCnt++;

	// Set row properties
	$internado_diario->ResetAttrs();
	$internado_diario->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$internado_diario_delete->LoadRowValues($internado_diario_delete->Recordset);

	// Render row
	$internado_diario_delete->RenderRow();
?>
	<tr<?php echo $internado_diario->RowAttributes() ?>>
<?php if ($internado_diario->idinternado_diario->Visible) { // idinternado_diario ?>
		<td<?php echo $internado_diario->idinternado_diario->CellAttributes() ?>>
<span id="el<?php echo $internado_diario_delete->RowCnt ?>_internado_diario_idinternado_diario" class="form-group internado_diario_idinternado_diario">
...
<span id="el<?php echo $internado_diario_delete->RowCnt ?>_internado_diario_idinternado" class="form-group internado_diario_idinternado">
...
<span id="el<?php echo $internado_diario_delete->RowCnt ?>_internado_diario_idcuenta" class="form-group internado_diario_idcuenta">
...
<span id="el<?php echo $internado_diario_delete->RowCnt ?>_internado_diario_fecha" class="form-group internado_diario_fecha">
...
<span id="el<?php echo $internado_diario_delete->RowCnt ?>_internado_diario_costo" class="form-group internado_diario_costo">
...
<?php
	$internado_diario_delete->Recordset->MoveNext();
}
$internado_diario_delete->Recordset->Close();
?>
</tbody>
...
<script type="text/javascript">
finternado_diariodelete.Init();
</script>
<?php
$internado_diario_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$internado_diario_delete->Page_Terminate();
?>
