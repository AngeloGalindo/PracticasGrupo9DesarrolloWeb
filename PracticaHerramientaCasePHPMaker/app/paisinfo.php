...
class cpais extends cTable {
	...
	function GetMasterFilter() {
		...
		if ($this->getCurrentMasterTable() == "continente") {
			if ($this->idcontinente->getSessionValue() <> "")
				$sMasterFilter .= "`idcontinente`=" . ew_QuotedValue($this->idcontinente->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "continente") {
			if ($this->idcontinente->getSessionValue() <> "")
				$sDetailFilter .= "`idcontinente`=" . ew_QuotedValue($this->idcontinente->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
	...
	// Master filter
	function SqlMasterFilter_continente() {
		return "`idcontinente`=@idcontinente@";
	}

	// Detail filter
	function SqlDetailFilter_continente() {
		return "`idcontinente`=@idcontinente@";
	}
	...
	function GetDetailUrl() {
		...
		// Detail url
		$sDetailUrl = "";
		if ($this->getCurrentDetailTable() == "departamento") {
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->nombre_oficial->Exportable) $Doc->ExportCaption($this->nombre_oficial);
					if ($this->gentilicio->Exportable) $Doc->ExportCaption($this->gentilicio);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->nombre_oficial->Exportable) $Doc->ExportField($this->nombre_oficial);
						if ($this->gentilicio->Exportable) $Doc->ExportField($this->gentilicio);
