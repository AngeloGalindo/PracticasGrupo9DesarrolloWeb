...
class cdepartamento extends cTable {
	...
	function __construct() {
		...
		// iddepartamento
		$this->iddepartamento = new cField('departamento', 'departamento', 'x_iddepartamento', 'iddepartamento', '`iddepartamento`', '`iddepartamento`', 3, -1, FALSE, '`iddepartamento`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->iddepartamento->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['iddepartamento'] = &$this->iddepartamento;

		// nombre
		$this->nombre = new cField('departamento', 'departamento', 'x_nombre', 'nombre', '`nombre`', '`nombre`', 200, -1, FALSE, '`nombre`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// idpais
		$this->idpais = new cField('departamento', 'departamento', 'x_idpais', 'idpais', '`idpais`', '`idpais`', 3, -1, FALSE, '`idpais`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('iddepartamento', $rs))
				ew_AddFilter($where, ew_QuotedName('iddepartamento') . '=' . ew_QuotedValue($rs['iddepartamento'], $this->iddepartamento->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("departamentoview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("departamentoview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "departamentoadd.php?" . $this->UrlParm($parm);
		else
			return "departamentoadd.php";
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["iddepartamento"]; // iddepartamento
