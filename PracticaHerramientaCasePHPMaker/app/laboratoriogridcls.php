...
$laboratorio_grid = NULL; // Initialize page object first

class claboratorio_grid extends claboratorio {
	...
	// Page object name
	var $PageObjName = 'laboratorio_grid';

	// Grid form hidden field names
	var $FormName = 'flaboratoriogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["laboratorio"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->idlaboratorio->CurrentValue;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->idlaboratorio->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		$this->idlaboratorio->CurrentValue = NULL;
		$this->idlaboratorio->OldValue = $this->idlaboratorio->CurrentValue;
	...
	function LoadFormValues() {
		...
		if (!$this->idlaboratorio->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idlaboratorio->CurrentValue = strval($arKeys[0]); // idlaboratorio
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
