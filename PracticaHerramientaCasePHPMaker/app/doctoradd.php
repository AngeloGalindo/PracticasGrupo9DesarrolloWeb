...
$doctor_add = NULL; // Initialize page object first

class cdoctor_add extends cdoctor {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["iddoctor"] != "") {
				...
				$this->setKey("iddoctor", $this->iddoctor->CurrentValue); // Set up key
			} else {
				$this->setKey("iddoctor", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "doctorview.php")
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->iddoctor->getSessionValue() <> "") {
			$rsnew['iddoctor'] = $this->iddoctor->getSessionValue();
		}
		...
		if ($AddRow) {
			$this->iddoctor->setDbValue($conn->Insert_ID());
			$rsnew['iddoctor'] = $this->iddoctor->DbValue;
		}
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("consulta", $DetailTblVar) && $GLOBALS["consulta"]->DetailAdd) {
				$GLOBALS["consulta"]->iddoctor->setSessionValue($this->iddoctor->CurrentValue); // Set master key
				if (!isset($GLOBALS["consulta_grid"])) $GLOBALS["consulta_grid"] = new cconsulta_grid(); // Get detail page object
				$AddRow = $GLOBALS["consulta_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["consulta"]->iddoctor->setSessionValue(""); // Clear master key if insert failed
			}
		}
...
// Create page object
if (!isset($doctor_add)) $doctor_add = new cdoctor_add();

// Page init
$doctor_add->Page_Init();

// Page main
$doctor_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$doctor_add->Page_Render();
?>
<?php include_once $EW_RELATIVE_PATH . "header.php" ?>
<script type="text/javascript">

// Page object
var doctor_add = new ew_Page("doctor_add");
doctor_add.PageID = "add"; // Page ID
var EW_PAGE_ID = doctor_add.PageID; // For backward compatibility

// Form object
var fdoctoradd = new ew_Form("fdoctoradd");

// Validate form
fdoctoradd.Validate = function() {
...
fdoctoradd.Form_CustomValidate = 
...
fdoctoradd.ValidateRequired = true;
<?php } else { ?>
fdoctoradd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fdoctoradd.Lists["x_idturno"] = {"LinkField":"x_idturno","Ajax":true,"AutoFill":false,"DisplayFields":["x_descripcion","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $doctor_add->ShowPageHeader(); ?>
<?php
$doctor_add->ShowMessage();
?>
<form name="fdoctoradd" id="fdoctoradd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($doctor_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $doctor_add->Token ?>">
<?php } ?>
...
<script type="text/javascript">
fdoctoradd.CreateAutoSuggest("x_idturno", true);
</script>
</span>
<?php } ?>
...
<?php if (strval($doctor->iddoctor->getSessionValue()) <> "") { ?>
<input type="hidden" name="x_iddoctor" id="x_iddoctor" value="<?php echo ew_HtmlEncode(strval($doctor->iddoctor->getSessionValue())) ?>">
<?php } ?>
<?php
	if (in_array("consulta", explode(",", $doctor->getCurrentDetailTable())) && $consulta->DetailAdd) {
?>
