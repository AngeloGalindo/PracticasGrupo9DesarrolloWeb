...
class cmunicipio_edit extends cmunicipio {
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->idmunicipio->CurrentValue == "")
	...
	function ValidateForm() {
		...
		if (in_array("hospital", $DetailTblVar) && $GLOBALS["hospital"]->DetailEdit) {
		...
		if ($rs->EOF) {
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					...
					if (in_array("hospital", $DetailTblVar) && $GLOBALS["hospital"]->DetailEdit) {
						if (!isset($GLOBALS["hospital_grid"])) $GLOBALS["hospital_grid"] = new chospital_grid(); // Get detail page object
						$EditRow = $GLOBALS["hospital_grid"]->GridUpdate();
					}
				}
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("hospital", $DetailTblVar)) {
				...
				if ($GLOBALS["hospital_grid"]->DetailEdit) {
					$GLOBALS["hospital_grid"]->CurrentMode = "edit";
					$GLOBALS["hospital_grid"]->CurrentAction = "gridedit";
...
// Form object
var fmunicipioedit = new ew_Form("fmunicipioedit");
...
fmunicipioedit.Lists["x_iddepartamento"] = {"LinkField":"x_iddepartamento","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
?>
<form name="fmunicipioedit" id="fmunicipioedit" class="form-horizontal ewForm ewEditForm" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<input type="hidden" data-field="x_idmunicipio" name="x_idmunicipio" id="x_idmunicipio" value="<?php echo ew_HtmlEncode($municipio->idmunicipio->CurrentValue) ?>">
<?php
	if (in_array("hospital", explode(",", $municipio->getCurrentDetailTable())) && $hospital->DetailEdit) {
?>
