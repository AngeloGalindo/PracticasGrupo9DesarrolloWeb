...
$doctor_servicio_medico_prestado_grid = NULL; // Initialize page object first

class cdoctor_servicio_medico_prestado_grid extends cdoctor_servicio_medico_prestado {
	...
	// Page object name
	var $PageObjName = 'doctor_servicio_medico_prestado_grid';

	// Grid form hidden field names
	var $FormName = 'fdoctor_servicio_medico_prestadogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["doctor_servicio_medico_prestado"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->iddoctor_servicio_medico_prestado->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_iddoctor") && $objForm->HasValue("o_iddoctor") && $this->iddoctor->CurrentValue <> $this->iddoctor->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_idservicio_medico_prestado") && $objForm->HasValue("o_idservicio_medico_prestado") && $this->idservicio_medico_prestado->CurrentValue <> $this->idservicio_medico_prestado->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->iddoctor_servicio_medico_prestado->CurrentValue . "\">";
		}
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('iddoctor_servicio_medico_prestado');
	}
	...
	function LoadDefaultValues() {
		$this->iddoctor_servicio_medico_prestado->CurrentValue = NULL;
		$this->iddoctor_servicio_medico_prestado->OldValue = $this->iddoctor_servicio_medico_prestado->CurrentValue;
		$this->iddoctor->CurrentValue = 1;
		$this->iddoctor->OldValue = $this->iddoctor->CurrentValue;
		$this->idservicio_medico_prestado->CurrentValue = 1;
		$this->idservicio_medico_prestado->OldValue = $this->idservicio_medico_prestado->CurrentValue;
	}
	...
	function LoadFormValues() {
		...
		if (!$this->iddoctor_servicio_medico_prestado->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
		...
		$this->iddoctor->setOldValue($objForm->GetValue("o_iddoctor"));
		...
		$this->idservicio_medico_prestado->setOldValue($objForm->GetValue("o_idservicio_medico_prestado"));
	}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->iddoctor_servicio_medico_prestado->CurrentValue = strval($arKeys[0]); // iddoctor_servicio_medico_prestado
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "servicio_medico_prestado") {
			$this->idservicio_medico_prestado->Visible = FALSE;
			if ($GLOBALS["servicio_medico_prestado"]->EventCancelled) $this->EventCancelled = TRUE;
		}
