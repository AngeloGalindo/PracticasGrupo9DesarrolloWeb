...
//
// Table class for internado
//
class cinternado extends cTable {
	var $idinternado;
	var $fecha_inicio;
	var $fecha_final;
	var $estado;
	...
	function __construct() {
		...
		$this->TableVar = 'internado';
		$this->TableName = 'internado';
		...
		// idinternado
		$this->idinternado = new cField('internado', 'internado', 'x_idinternado', 'idinternado', '`idinternado`', '`idinternado`', 3, -1, FALSE, '`idinternado`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->idinternado->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['idinternado'] = &$this->idinternado;

		// fecha_inicio
		$this->fecha_inicio = new cField('internado', 'internado', 'x_fecha_inicio', 'fecha_inicio', '`fecha_inicio`', 'DATE_FORMAT(`fecha_inicio`, \'%d/%m/%Y\')', 135, 7, FALSE, '`fecha_inicio`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// fecha_final
		$this->fecha_final = new cField('internado', 'internado', 'x_fecha_final', 'fecha_final', '`fecha_final`', 'DATE_FORMAT(`fecha_final`, \'%d/%m/%Y\')', 135, 7, FALSE, '`fecha_final`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fecha_final->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['fecha_final'] = &$this->fecha_final;

		// estado
		$this->estado = new cField('internado', 'internado', 'x_estado', 'estado', '`estado`', '`estado`', 202, -1, FALSE, '`estado`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['estado'] = &$this->estado;

		// fecha
		$this->fecha = new cField('internado', 'internado', 'x_fecha', 'fecha', '`fecha`', 'DATE_FORMAT(`fecha`, \'%d/%m/%Y\')', 135, 7, FALSE, '`fecha`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fecha->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['fecha'] = &$this->fecha;

		// idhabitacion
		$this->idhabitacion = new cField('internado', 'internado', 'x_idhabitacion', 'idhabitacion', '`idhabitacion`', '`idhabitacion`', 3, -1, FALSE, '`idhabitacion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->idhabitacion->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['idhabitacion'] = &$this->idhabitacion;

		// idpaciente
		$this->idpaciente = new cField('internado', 'internado', 'x_idpaciente', 'idpaciente', '`idpaciente`', '`idpaciente`', 3, -1, FALSE, '`idpaciente`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->idpaciente->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['idpaciente'] = &$this->idpaciente;

		// es_operacion
		$this->es_operacion = new cField('internado', 'internado', 'x_es_operacion', 'es_operacion', '`es_operacion`', '`es_operacion`', 202, -1, FALSE, '`es_operacion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['es_operacion'] = &$this->es_operacion;
	}
	...
	function GetMasterFilter() {
		...
		// Master filter
		$sMasterFilter = "";
		if ($this->getCurrentMasterTable() == "habitacion") {
			if ($this->idhabitacion->getSessionValue() <> "")
				$sMasterFilter .= "`idhabitacion`=" . ew_QuotedValue($this->idhabitacion->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
		if ($this->getCurrentMasterTable() == "paciente") {
			if ($this->idpaciente->getSessionValue() <> "")
				$sMasterFilter .= "`idpaciente`=" . ew_QuotedValue($this->idpaciente->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "habitacion") {
			if ($this->idhabitacion->getSessionValue() <> "")
				$sDetailFilter .= "`idhabitacion`=" . ew_QuotedValue($this->idhabitacion->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
		if ($this->getCurrentMasterTable() == "paciente") {
			if ($this->idpaciente->getSessionValue() <> "")
				$sDetailFilter .= "`idpaciente`=" . ew_QuotedValue($this->idpaciente->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
	...
	// Master filter
	function SqlMasterFilter_habitacion() {
		return "`idhabitacion`=@idhabitacion@";
	}

	// Detail filter
	function SqlDetailFilter_habitacion() {
		return "`idhabitacion`=@idhabitacion@";
	}

	// Master filter
	function SqlMasterFilter_paciente() {
		return "`idpaciente`=@idpaciente@";
	}

	// Detail filter
	function SqlDetailFilter_paciente() {
		return "`idpaciente`=@idpaciente@";
	}
	...
	function GetDetailUrl() {
		...
		// Detail url
		$sDetailUrl = "";
		if ($this->getCurrentDetailTable() == "internado_diario") {
			$sDetailUrl = $GLOBALS["internado_diario"]->GetListUrl() . "?showmaster=" . $this->TableVar;
			$sDetailUrl .= "&fk_idinternado=" . urlencode($this->idinternado->CurrentValue);
		}
		if ($sDetailUrl == "") {
			$sDetailUrl = "internadolist.php";
		}
		return $sDetailUrl;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`internado`";
	}
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('idinternado', $rs))
				ew_AddFilter($where, ew_QuotedName('idinternado') . '=' . ew_QuotedValue($rs['idinternado'], $this->idinternado->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`idinternado` = @idinternado@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->idinternado->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@idinternado@", ew_AdjustSql($this->idinternado->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("internadoview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("internadoview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "internadoadd.php?" . $this->UrlParm($parm);
		else
			return "internadoadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("internadoedit.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("internadoedit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("internadoadd.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("internadoadd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("internadodelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->idinternado->CurrentValue)) {
			$sUrl .= "idinternado=" . urlencode($this->idinternado->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["idinternado"]; // idinternado
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->idinternado->CurrentValue = $key;
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->idinternado->EditValue = $this->idinternado->CurrentValue;
		$this->idinternado->ViewCustomAttributes = "";
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->idinternado->Exportable) $Doc->ExportCaption($this->idinternado);
					...
					if ($this->idhabitacion->Exportable) $Doc->ExportCaption($this->idhabitacion);
					if ($this->idpaciente->Exportable) $Doc->ExportCaption($this->idpaciente);
					if ($this->es_operacion->Exportable) $Doc->ExportCaption($this->es_operacion);
				} else {
					if ($this->idinternado->Exportable) $Doc->ExportCaption($this->idinternado);
					if ($this->fecha_inicio->Exportable) $Doc->ExportCaption($this->fecha_inicio);
					if ($this->fecha_final->Exportable) $Doc->ExportCaption($this->fecha_final);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->idinternado->Exportable) $Doc->ExportField($this->idinternado);
						...
						if ($this->idhabitacion->Exportable) $Doc->ExportField($this->idhabitacion);
						if ($this->idpaciente->Exportable) $Doc->ExportField($this->idpaciente);
						if ($this->es_operacion->Exportable) $Doc->ExportField($this->es_operacion);
					} else {
						if ($this->idinternado->Exportable) $Doc->ExportField($this->idinternado);
						if ($this->fecha_inicio->Exportable) $Doc->ExportField($this->fecha_inicio);
						if ($this->fecha_final->Exportable) $Doc->ExportField($this->fecha_final);
