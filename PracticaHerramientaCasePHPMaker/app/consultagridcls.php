...
$consulta_grid = NULL; // Initialize page object first

class cconsulta_grid extends cconsulta {
	...
	// Page object name
	var $PageObjName = 'consulta_grid';

	// Grid form hidden field names
	var $FormName = 'fconsultagrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["consulta"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "doctor") {
			global $doctor;
			$rsmaster = $doctor->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("doctorlist.php"); // Return to master page
			} else {
				$doctor->LoadListRowValues($rsmaster);
				$doctor->RowType = EW_ROWTYPE_MASTER; // Master row
				$doctor->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->idconsulta->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->idconsulta->FormValue))
				return FALSE;
		}
		return TRUE;
	}
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->idconsulta->CurrentValue;
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_idcuenta") && $objForm->HasValue("o_idcuenta") && $this->idcuenta->CurrentValue <> $this->idcuenta->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_fecha_cita") && $objForm->HasValue("o_fecha_cita") && $this->fecha_cita->CurrentValue <> $this->fecha_cita->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->idconsulta->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		$this->idconsulta->CurrentValue = NULL;
		$this->idconsulta->OldValue = $this->idconsulta->CurrentValue;
		...
		$this->estado->CurrentValue = 'Espera';
	...
	function LoadFormValues() {
		...
		if (!$this->idconsulta->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
		...
		$this->fecha_cita->setOldValue($objForm->GetValue("o_fecha_cita"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idconsulta->CurrentValue = strval($arKeys[0]); // idconsulta
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "doctor") {
			...
			if ($GLOBALS["doctor"]->EventCancelled) $this->EventCancelled = TRUE;
		}
