...
class cpais_list extends cpais {
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "continente") {
			global $continente;
			$rsmaster = $continente->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("continentelist.php"); // Return to master page
			} else {
				$continente->LoadListRowValues($rsmaster);
				$continente->RowType = EW_ROWTYPE_MASTER; // Master row
				$continente->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function BasicSearchSQL($arKeywords, $type) {
		...
		$this->BuildBasicSearchSQL($sWhere, $this->nombre_oficial, $arKeywords, $type);
		$this->BuildBasicSearchSQL($sWhere, $this->gentilicio, $arKeywords, $type);
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->nombre_oficial); // nombre oficial
			$this->UpdateSort($this->gentilicio); // gentilicio
			$this->UpdateSort($this->flag); // flag
			$this->UpdateSort($this->idcontinente); // idcontinente
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetall") {
				...
				$this->DbMasterFilter = "";
				$this->DbDetailFilter = "";
				$this->idcontinente->setSessionValue("");
			}
			...
			if ($this->Command == "resetsort") {
				...
				$this->setSessionOrderBy($sOrderBy);
				$this->nombre->setSort("");
				$this->nombre_oficial->setSort("");
				$this->gentilicio->setSort("");
				$this->flag->setSort("");
				$this->idcontinente->setSort("");
				$this->estado->setSort("");
			}
	...
	function SetupListOptions() {
		...
		// "detail_laboratorio"
		$item = &$this->ListOptions->Add("detail_laboratorio");
		$item->CssStyle = "white-space: nowrap;";
		$item->Visible = TRUE && !$this->ShowMultipleDetails;
		$item->OnLeft = FALSE;
		$item->ShowInButtonGroup = FALSE;
	...
	function RenderListOptions() {
		...
		// "detail_laboratorio"
		$oListOpt = &$this->ListOptions->Items["detail_laboratorio"];
		if (TRUE) {
	...
	function SetupOtherOptions() {
		...
		$item = &$option->Add("detailadd_laboratorio");
		$item->Body = "<a class=\"ewDetailAddGroup ewDetailAdd\" title=\"" . ew_HtmlTitle($Language->Phrase("AddMasterDetailLink")) . "\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("AddMasterDetailLink")) . "\" href=\"" . ew_HtmlEncode($this->GetAddUrl() . "?" . EW_TABLE_SHOW_DETAIL . "=laboratorio") . "\">" . $Language->Phrase("Add") . "&nbsp;" . $this->TableCaption() . "/" . $GLOBALS["laboratorio"]->TableCaption() . "</a>";
		$item->Visible = ($GLOBALS["laboratorio"]->DetailAdd);
		if ($item->Visible) {
			if ($DetailTableLink <> "") $DetailTableLink .= ",";
...
fpaislist.Lists["x_idcontinente"] = {"LinkField":"x_idcontinente","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php if ($pais_list->TotalRecs > 0 && $pais->getCurrentMasterTable() == "" && $pais_list->ExportOptions->Visible()) { ?>
...
<?php if (($pais->Export == "") || (EW_EXPORT_MASTER_RECORD && $pais->Export == "print")) { ?>
<?php
$gsMasterReturnUrl = "continentelist.php";
if ($pais_list->DbMasterFilter <> "" && $pais->getCurrentMasterTable() == "continente") {
	if ($pais_list->MasterRecordExists) {
		if ($pais->getCurrentMasterTable() == $pais->TableVar) $gsMasterReturnUrl .= "?" . EW_TABLE_SHOW_MASTER . "=";
?>
<?php if ($pais_list->ExportOptions->Visible()) { ?>
<div class="ewListExportOptions"><?php $pais_list->ExportOptions->Render("body") ?></div>
<?php } ?>
<?php include_once $EW_RELATIVE_PATH . "continentemaster.php" ?>
<?php
	}
}
?>
...
<?php if ($pais->nombre_oficial->Visible) { // nombre oficial ?>
	...
	<?php } else { ?>
		<th data-name="nombre_oficial"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pais->SortUrl($pais->nombre_oficial) ?>',1);"><div id="elh_pais_nombre_oficial" class="pais_nombre_oficial">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pais->nombre_oficial->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($pais->nombre_oficial->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pais->nombre_oficial->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($pais->gentilicio->Visible) { // gentilicio ?>
	...
	<?php } else { ?>
		<th data-name="gentilicio"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pais->SortUrl($pais->gentilicio) ?>',1);"><div id="elh_pais_gentilicio" class="pais_gentilicio">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pais->gentilicio->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($pais->gentilicio->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pais->gentilicio->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($pais->flag->Visible) { // flag ?>
	...
	<?php } else { ?>
		<th data-name="flag"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pais->SortUrl($pais->flag) ?>',1);"><div id="elh_pais_flag" class="pais_flag">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pais->flag->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($pais->flag->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pais->flag->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($pais->idcontinente->Visible) { // idcontinente ?>
	...
	<?php } else { ?>
		<th data-name="idcontinente"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pais->SortUrl($pais->idcontinente) ?>',1);"><div id="elh_pais_idcontinente" class="pais_idcontinente">
...
<?php if ($pais->estado->Visible) { // estado ?>
	...
	<?php } else { ?>
		<th data-name="estado"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pais->SortUrl($pais->estado) ?>',1);"><div id="elh_pais_estado" class="pais_estado">
