...
$departamento_grid = NULL; // Initialize page object first

class cdepartamento_grid extends cdepartamento {
	...
	// Page object name
	var $PageObjName = 'departamento_grid';

	// Grid form hidden field names
	var $FormName = 'fdepartamentogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["departamento"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->iddepartamento->CurrentValue;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->iddepartamento->CurrentValue . "\">";
		}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->iddepartamento->CurrentValue = strval($arKeys[0]); // iddepartamento
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
