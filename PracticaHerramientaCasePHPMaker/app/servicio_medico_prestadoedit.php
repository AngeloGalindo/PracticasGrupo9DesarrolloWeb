...
$servicio_medico_prestado_edit = NULL; // Initialize page object first

class cservicio_medico_prestado_edit extends cservicio_medico_prestado {
	...
	// Page object name
	var $PageObjName = 'servicio_medico_prestado_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->idservicio_medico_prestado->CurrentValue == "")
			$this->Page_Terminate("servicio_medico_prestadolist.php"); // Invalid key, return to list
	...
	function ValidateForm() {
		...
		if (in_array("doctor_servicio_medico_prestado", $DetailTblVar) && $GLOBALS["doctor_servicio_medico_prestado"]->DetailEdit) {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// idcuenta
			$this->idcuenta->SetDbValueDef($rsnew, $this->idcuenta->CurrentValue, NULL, $this->idcuenta->ReadOnly);

			// idservicio_medico
			$this->idservicio_medico->SetDbValueDef($rsnew, $this->idservicio_medico->CurrentValue, 0, $this->idservicio_medico->ReadOnly);

			// estado
			$this->estado->SetDbValueDef($rsnew, $this->estado->CurrentValue, "", $this->estado->ReadOnly);

			// costo
			$this->costo->SetDbValueDef($rsnew, $this->costo->CurrentValue, 0, $this->costo->ReadOnly);

			// Check referential integrity for master table 'servicio_medico'
			$bValidMasterRecord = TRUE;
			$sMasterFilter = $this->SqlMasterFilter_servicio_medico();
			$KeyValue = isset($rsnew['idservicio_medico']) ? $rsnew['idservicio_medico'] : $rsold['idservicio_medico'];
			if (strval($KeyValue) <> "") {
				$sMasterFilter = str_replace("@idservicio_medico@", ew_AdjustSql($KeyValue), $sMasterFilter);
			} else {
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					...
					if (in_array("doctor_servicio_medico_prestado", $DetailTblVar) && $GLOBALS["doctor_servicio_medico_prestado"]->DetailEdit) {
						if (!isset($GLOBALS["doctor_servicio_medico_prestado_grid"])) $GLOBALS["doctor_servicio_medico_prestado_grid"] = new cdoctor_servicio_medico_prestado_grid(); // Get detail page object
						$EditRow = $GLOBALS["doctor_servicio_medico_prestado_grid"]->GridUpdate();
					}
				}
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("doctor_servicio_medico_prestado", $DetailTblVar)) {
				...
				if ($GLOBALS["doctor_servicio_medico_prestado_grid"]->DetailEdit) {
					$GLOBALS["doctor_servicio_medico_prestado_grid"]->CurrentMode = "edit";
					$GLOBALS["doctor_servicio_medico_prestado_grid"]->CurrentAction = "gridedit";
...
// Create page object
if (!isset($servicio_medico_prestado_edit)) $servicio_medico_prestado_edit = new cservicio_medico_prestado_edit();

// Page init
$servicio_medico_prestado_edit->Page_Init();

// Page main
$servicio_medico_prestado_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$servicio_medico_prestado_edit->Page_Render();
?>
<?php include_once $EW_RELATIVE_PATH . "header.php" ?>
<script type="text/javascript">

// Page object
var servicio_medico_prestado_edit = new ew_Page("servicio_medico_prestado_edit");
servicio_medico_prestado_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = servicio_medico_prestado_edit.PageID; // For backward compatibility

// Form object
var fservicio_medico_prestadoedit = new ew_Form("fservicio_medico_prestadoedit");

// Validate form
fservicio_medico_prestadoedit.Validate = function() {
...
fservicio_medico_prestadoedit.Form_CustomValidate = 
...
fservicio_medico_prestadoedit.ValidateRequired = true;
<?php } else { ?>
fservicio_medico_prestadoedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fservicio_medico_prestadoedit.Lists["x_idcuenta"] = {"LinkField":"x_idcuenta","Ajax":true,"AutoFill":false,"DisplayFields":["x_idcuenta","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fservicio_medico_prestadoedit.Lists["x_idservicio_medico"] = {"LinkField":"x_idservicio_medico","Ajax":true,"AutoFill":false,"DisplayFields":["x_descripcion","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $servicio_medico_prestado_edit->ShowPageHeader(); ?>
<?php
$servicio_medico_prestado_edit->ShowMessage();
?>
<form name="fservicio_medico_prestadoedit" id="fservicio_medico_prestadoedit" class="form-horizontal ewForm ewEditForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($servicio_medico_prestado_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $servicio_medico_prestado_edit->Token ?>">
<?php } ?>
...
<?php if ($servicio_medico_prestado->estado->Visible) { // estado ?>
	<div id="r_estado" class="form-group">
		<label id="elh_servicio_medico_prestado_estado" class="col-sm-2 control-label ewLabel"><?php echo $servicio_medico_prestado->estado->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $servicio_medico_prestado->estado->CellAttributes() ?>>
<span id="el_servicio_medico_prestado_estado">
...
<?php echo $servicio_medico_prestado->estado->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<input type="hidden" data-field="x_idservicio_medico_prestado" name="x_idservicio_medico_prestado" id="x_idservicio_medico_prestado" value="<?php echo ew_HtmlEncode($servicio_medico_prestado->idservicio_medico_prestado->CurrentValue) ?>">
<?php
	if (in_array("doctor_servicio_medico_prestado", explode(",", $servicio_medico_prestado->getCurrentDetailTable())) && $doctor_servicio_medico_prestado->DetailEdit) {
?>
...
<script type="text/javascript">
fservicio_medico_prestadoedit.Init();
</script>
<?php
$servicio_medico_prestado_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$servicio_medico_prestado_edit->Page_Terminate();
?>
