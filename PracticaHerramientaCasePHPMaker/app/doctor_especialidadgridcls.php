...
$doctor_especialidad_grid = NULL; // Initialize page object first

class cdoctor_especialidad_grid extends cdoctor_especialidad {
	...
	// Page object name
	var $PageObjName = 'doctor_especialidad_grid';

	// Grid form hidden field names
	var $FormName = 'fdoctor_especialidadgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["doctor_especialidad"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->iddoctor_especialidad->CurrentValue;
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_iddoctor") && $objForm->HasValue("o_iddoctor") && $this->iddoctor->CurrentValue <> $this->iddoctor->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_idespecialidad") && $objForm->HasValue("o_idespecialidad") && $this->idespecialidad->CurrentValue <> $this->idespecialidad->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->iddoctor_especialidad->CurrentValue . "\">";
		}
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('iddoctor_especialidad');
	}
	...
	function LoadDefaultValues() {
		$this->iddoctor_especialidad->CurrentValue = NULL;
		$this->iddoctor_especialidad->OldValue = $this->iddoctor_especialidad->CurrentValue;
		$this->iddoctor->CurrentValue = 1;
		$this->iddoctor->OldValue = $this->iddoctor->CurrentValue;
		$this->idespecialidad->CurrentValue = 1;
		$this->idespecialidad->OldValue = $this->idespecialidad->CurrentValue;
	}
	...
	function LoadFormValues() {
		...
		if (!$this->iddoctor_especialidad->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
		...
		$this->idespecialidad->setOldValue($objForm->GetValue("o_idespecialidad"));
	}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->iddoctor_especialidad->CurrentValue = strval($arKeys[0]); // iddoctor_especialidad
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "especialidad") {
			...
			if ($GLOBALS["especialidad"]->EventCancelled) $this->EventCancelled = TRUE;
		}
