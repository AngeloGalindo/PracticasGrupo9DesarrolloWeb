...
//
// Table class for municipio
//
class cmunicipio extends cTable {
	var $idmunicipio;
	var $nombre;
	var $estado;
	var $iddepartamento;
	...
	function __construct() {
		...
		// idmunicipio
		$this->idmunicipio = new cField('municipio', 'municipio', 'x_idmunicipio', 'idmunicipio', '`idmunicipio`', '`idmunicipio`', 3, -1, FALSE, '`idmunicipio`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->idmunicipio->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['idmunicipio'] = &$this->idmunicipio;

		// nombre
		$this->nombre = new cField('municipio', 'municipio', 'x_nombre', 'nombre', '`nombre`', '`nombre`', 200, -1, FALSE, '`nombre`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// iddepartamento
		$this->iddepartamento = new cField('municipio', 'municipio', 'x_iddepartamento', 'iddepartamento', '`iddepartamento`', '`iddepartamento`', 3, -1, FALSE, '`iddepartamento`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetMasterFilter() {
		...
		if ($this->getCurrentMasterTable() == "departamento") {
			if ($this->iddepartamento->getSessionValue() <> "")
				$sMasterFilter .= "`idpais`=" . ew_QuotedValue($this->iddepartamento->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
	...
	function GetDetailUrl() {
		...
		// Detail url
		$sDetailUrl = "";
		if ($this->getCurrentDetailTable() == "hospital") {
			$sDetailUrl = $GLOBALS["hospital"]->GetListUrl() . "?showmaster=" . $this->TableVar;
			$sDetailUrl .= "&fk_idmunicipio=" . urlencode($this->idmunicipio->CurrentValue);
		}
		if ($sDetailUrl == "") {
			$sDetailUrl = "municipiolist.php";
		}
		return $sDetailUrl;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`municipio`";
	}
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('idmunicipio', $rs))
				ew_AddFilter($where, ew_QuotedName('idmunicipio') . '=' . ew_QuotedValue($rs['idmunicipio'], $this->idmunicipio->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("municipioview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("municipioview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "municipioadd.php?" . $this->UrlParm($parm);
		else
			return "municipioadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("municipioedit.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("municipioedit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("municipioadd.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("municipioadd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["idmunicipio"]; // idmunicipio
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->idmunicipio->CurrentValue = $key;
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->idmunicipio->EditValue = $this->idmunicipio->CurrentValue;
		$this->idmunicipio->ViewCustomAttributes = "";
