...
$turno_grid = NULL; // Initialize page object first

class cturno_grid extends cturno {
	...
	// Page object name
	var $PageObjName = 'turno_grid';

	// Grid form hidden field names
	var $FormName = 'fturnogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["turno"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "tipo_turno") {
			global $tipo_turno;
			$rsmaster = $tipo_turno->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("tipo_turnolist.php"); // Return to master page
			} else {
				$tipo_turno->LoadListRowValues($rsmaster);
				$tipo_turno->RowType = EW_ROWTYPE_MASTER; // Master row
				$tipo_turno->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->idturno->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->idturno->FormValue))
				return FALSE;
		}
		return TRUE;
	}
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->idturno->CurrentValue;
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_descripcion") && $objForm->HasValue("o_descripcion") && $this->descripcion->CurrentValue <> $this->descripcion->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_idtipo_turno") && $objForm->HasValue("o_idtipo_turno") && $this->idtipo_turno->CurrentValue <> $this->idtipo_turno->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_idhospital") && $objForm->HasValue("o_idhospital") && $this->idhospital->CurrentValue <> $this->idhospital->OldValue)
			return FALSE;
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetall") {
				...
				$this->idtipo_turno->setSessionValue("");
			}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->idturno->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		...
		$this->idtipo_turno->OldValue = $this->idtipo_turno->CurrentValue;
		$this->idhospital->CurrentValue = 1;
		$this->idhospital->OldValue = $this->idhospital->CurrentValue;
	...
	function LoadFormValues() {
		...
		$this->idtipo_turno->setOldValue($objForm->GetValue("o_idtipo_turno"));
		if (!$this->idhospital->FldIsDetailKey) {
			$this->idhospital->setFormValue($objForm->GetValue("x_idhospital"));
		}
		$this->idhospital->setOldValue($objForm->GetValue("o_idhospital"));
		if (!$this->estado->FldIsDetailKey) {
			$this->estado->setFormValue($objForm->GetValue("x_estado"));
		}
		$this->estado->setOldValue($objForm->GetValue("o_estado"));
		if (!$this->idturno->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idturno->CurrentValue = strval($arKeys[0]); // idturno
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "hospital") {
			...
			if ($GLOBALS["hospital"]->EventCancelled) $this->EventCancelled = TRUE;
		}
		if ($sMasterTblVar == "tipo_turno") {
			$this->idtipo_turno->Visible = FALSE;
			if ($GLOBALS["tipo_turno"]->EventCancelled) $this->EventCancelled = TRUE;
		}
