...
$doctor_grid = NULL; // Initialize page object first

class cdoctor_grid extends cdoctor {
	...
	// Page object name
	var $PageObjName = 'doctor_grid';

	// Grid form hidden field names
	var $FormName = 'fdoctorgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["doctor"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->iddoctor->CurrentValue;
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_apellido") && $objForm->HasValue("o_apellido") && $this->apellido->CurrentValue <> $this->apellido->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_cui") && $objForm->HasValue("o_cui") && $this->cui->CurrentValue <> $this->cui->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->iddoctor->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->cui->setOldValue($objForm->GetValue("o_cui"));
		...
		if (!$this->iddoctor->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->iddoctor->CurrentValue = strval($arKeys[0]); // iddoctor
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "doctor_especialidad") {
			...
			if ($GLOBALS["doctor_especialidad"]->EventCancelled) $this->EventCancelled = TRUE;
		}
