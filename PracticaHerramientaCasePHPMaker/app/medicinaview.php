...
$medicina_view = NULL; // Initialize page object first

class cmedicina_view extends cmedicina {
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["idmedicina"] <> "") {
			$this->RecKey["idmedicina"] = $_GET["idmedicina"];
			$KeyUrl .= "&amp;idmedicina=" . urlencode($this->RecKey["idmedicina"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["idmedicina"] <> "") {
				...
				$this->RecKey["idmedicina"] = $this->idmedicina->QueryStringValue;
			} else {
				$sReturnUrl = "medicinalist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "medicinalist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "medicinalist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupOtherOptions() {
		...
		$body = "<a class=\"btn btn-default btn-sm ewRowLink ewDetail\" data-action=\"list\" href=\"" . ew_HtmlEncode("recetalist.php?" . EW_TABLE_SHOW_MASTER . "=medicina&fk_idmedicina=" . strval($this->idmedicina->CurrentValue) . "") . "\">" . $body . "</a>";
		$links = "";
...
// Create page object
if (!isset($medicina_view)) $medicina_view = new cmedicina_view();

// Page init
$medicina_view->Page_Init();

// Page main
$medicina_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$medicina_view->Page_Render();
?>
<?php include_once $EW_RELATIVE_PATH . "header.php" ?>
<script type="text/javascript">

// Page object
var medicina_view = new ew_Page("medicina_view");
medicina_view.PageID = "view"; // Page ID
var EW_PAGE_ID = medicina_view.PageID; // For backward compatibility

// Form object
var fmedicinaview = new ew_Form("fmedicinaview");

// Form_CustomValidate event
fmedicinaview.Form_CustomValidate = 
...
fmedicinaview.ValidateRequired = true;
<?php } else { ?>
fmedicinaview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fmedicinaview.Lists["x_idlaboratorio"] = {"LinkField":"x_idlaboratorio","Ajax":true,"AutoFill":false,"DisplayFields":["x_descripcion","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $medicina_view->ExportOptions->Render("body") ?>
<?php
	foreach ($medicina_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php echo $Language->SelectionForm(); ?>
<div class="clearfix"></div>
</div>
<?php $medicina_view->ShowPageHeader(); ?>
<?php
$medicina_view->ShowMessage();
?>
<form name="fmedicinaview" id="fmedicinaview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($medicina_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $medicina_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="medicina">
<table class="table table-bordered table-striped ewViewTable">
<?php if ($medicina->idmedicina->Visible) { // idmedicina ?>
	<tr id="r_idmedicina">
		<td><span id="elh_medicina_idmedicina"><?php echo $medicina->idmedicina->FldCaption() ?></span></td>
...
<?php echo $medicina->idmedicina->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($medicina->descripcion->Visible) { // descripcion ?>
	<tr id="r_descripcion">
		<td><span id="elh_medicina_descripcion"><?php echo $medicina->descripcion->FldCaption() ?></span></td>
...
<?php echo $medicina->descripcion->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($medicina->estado->Visible) { // estado ?>
	<tr id="r_estado">
		<td><span id="elh_medicina_estado"><?php echo $medicina->estado->FldCaption() ?></span></td>
...
<?php echo $medicina->estado->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($medicina->idlaboratorio->Visible) { // idlaboratorio ?>
	<tr id="r_idlaboratorio">
		<td><span id="elh_medicina_idlaboratorio"><?php echo $medicina->idlaboratorio->FldCaption() ?></span></td>
...
<?php echo $medicina->idlaboratorio->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($medicina->idhospital->Visible) { // idhospital ?>
	<tr id="r_idhospital">
		<td><span id="elh_medicina_idhospital"><?php echo $medicina->idhospital->FldCaption() ?></span></td>
...
<?php echo $medicina->idhospital->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php
	if (in_array("receta", explode(",", $medicina->getCurrentDetailTable())) && $receta->DetailView) {
?>
