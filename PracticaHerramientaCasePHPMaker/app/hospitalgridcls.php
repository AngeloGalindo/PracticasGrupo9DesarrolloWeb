...
$hospital_grid = NULL; // Initialize page object first

class chospital_grid extends chospital {
	...
	// Page object name
	var $PageObjName = 'hospital_grid';

	// Grid form hidden field names
	var $FormName = 'fhospitalgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["hospital"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->idhospital->CurrentValue;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->idhospital->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		if (!$this->idhospital->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idhospital->CurrentValue = strval($arKeys[0]); // idhospital
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->idmunicipio->getSessionValue() <> "") {
			$rsnew['idmunicipio'] = $this->idmunicipio->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "municipio") {
			...
			if ($GLOBALS["municipio"]->EventCancelled) $this->EventCancelled = TRUE;
		}
