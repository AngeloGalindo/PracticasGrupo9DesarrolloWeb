...
$hospital_list = NULL; // Initialize page object first

class chospital_list extends chospital {
	...
	// Page object name
	var $PageObjName = 'hospital_list';

	// Grid form hidden field names
	var $FormName = 'fhospitallist';
	...
	function __construct() {
		...
		$this->AddUrl = "hospitaladd.php?" . EW_TABLE_SHOW_DETAIL . "=";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "hospitaldelete.php";
		$this->MultiUpdateUrl = "hospitalupdate.php";
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "municipio") {
			global $municipio;
			$rsmaster = $municipio->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("municipiolist.php"); // Return to master page
			} else {
				$municipio->LoadListRowValues($rsmaster);
				$municipio->RowType = EW_ROWTYPE_MASTER; // Master row
				$municipio->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->idhospital->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->idhospital->FormValue))
				return FALSE;
		}
		return TRUE;
	}
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetall") {
				...
				$this->DbMasterFilter = "";
				$this->DbDetailFilter = "";
				$this->idmunicipio->setSessionValue("");
			}
	...
	function SetupListOptions() {
		...
		// "detail_nivel"
		$item = &$this->ListOptions->Add("detail_nivel");
		$item->CssStyle = "white-space: nowrap;";
		...
		// "detail_turno"
		$item = &$this->ListOptions->Add("detail_turno");
		$item->CssStyle = "white-space: nowrap;";
		$item->Visible = TRUE && !$this->ShowMultipleDetails;
		$item->OnLeft = FALSE;
		$item->ShowInButtonGroup = FALSE;
	...
	function RenderListOptions() {
		...
		// "detail_nivel"
		$oListOpt = &$this->ListOptions->Items["detail_nivel"];
		if (TRUE) {
		...
		// "detail_turno"
		$oListOpt = &$this->ListOptions->Items["detail_turno"];
		if (TRUE) {
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" value=\"" . ew_HtmlEncode($this->idhospital->CurrentValue) . "\" onclick='ew_ClickMultiCheckbox(event, this);'>";
	...
	function SetupOtherOptions() {
		...
		$DetailTableLink = "";
		$item = &$option->Add("detailadd_nivel");
		$item->Body = "<a class=\"ewDetailAddGroup ewDetailAdd\" title=\"" . ew_HtmlTitle($Language->Phrase("AddMasterDetailLink")) . "\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("AddMasterDetailLink")) . "\" href=\"" . ew_HtmlEncode($this->GetAddUrl() . "?" . EW_TABLE_SHOW_DETAIL . "=nivel") . "\">" . $Language->Phrase("Add") . "&nbsp;" . $this->TableCaption() . "/" . $GLOBALS["nivel"]->TableCaption() . "</a>";
		$item->Visible = ($GLOBALS["nivel"]->DetailAdd);
		if ($item->Visible) {
			if ($DetailTableLink <> "") $DetailTableLink .= ",";
			$DetailTableLink .= "nivel";
		}
		$item = &$option->Add("detailadd_turno");
		$item->Body = "<a class=\"ewDetailAddGroup ewDetailAdd\" title=\"" . ew_HtmlTitle($Language->Phrase("AddMasterDetailLink")) . "\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("AddMasterDetailLink")) . "\" href=\"" . ew_HtmlEncode($this->GetAddUrl() . "?" . EW_TABLE_SHOW_DETAIL . "=turno") . "\">" . $Language->Phrase("Add") . "&nbsp;" . $this->TableCaption() . "/" . $GLOBALS["turno"]->TableCaption() . "</a>";
		$item->Visible = ($GLOBALS["turno"]->DetailAdd);
		if ($item->Visible) {
			if ($DetailTableLink <> "") $DetailTableLink .= ",";
	...
	function RenderOtherOptions() {
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->CustomActions as $action => $name) {
				...
				$item->Body = "<a class=\"ewAction ewCustomAction\" href=\"\" onclick=\"ew_SubmitSelected(document.fhospitallist, '" . ew_CurrentUrl() . "', null, '" . $action . "');return false;\">" . $name . "</a>";
			}
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fhospitallistsrch\">" . $Language->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($hospital_list)) $hospital_list = new chospital_list();

// Page init
$hospital_list->Page_Init();

// Page main
$hospital_list->Page_Main();
...
// Page object
var hospital_list = new ew_Page("hospital_list");
hospital_list.PageID = "list"; // Page ID
var EW_PAGE_ID = hospital_list.PageID; // For backward compatibility

// Form object
var fhospitallist = new ew_Form("fhospitallist");
fhospitallist.FormKeyCountName = '<?php echo $hospital_list->FormKeyCountName ?>';

// Form_CustomValidate event
fhospitallist.Form_CustomValidate = 
...
fhospitallist.ValidateRequired = true;
<?php } else { ?>
fhospitallist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
// Form object for search

var fhospitallistsrch = new ew_Form("fhospitallistsrch");
</script>
<script type="text/javascript">
...
<?php if ($hospital_list->TotalRecs > 0 && $hospital->getCurrentMasterTable() == "" && $hospital_list->ExportOptions->Visible()) { ?>
...
<?php if (($hospital->Export == "") || (EW_EXPORT_MASTER_RECORD && $hospital->Export == "print")) { ?>
<?php
$gsMasterReturnUrl = "municipiolist.php";
if ($hospital_list->DbMasterFilter <> "" && $hospital->getCurrentMasterTable() == "municipio") {
	if ($hospital_list->MasterRecordExists) {
		if ($hospital->getCurrentMasterTable() == $hospital->TableVar) $gsMasterReturnUrl .= "?" . EW_TABLE_SHOW_MASTER . "=";
?>
<?php if ($hospital_list->ExportOptions->Visible()) { ?>
<div class="ewListExportOptions"><?php $hospital_list->ExportOptions->Render("body") ?></div>
<?php } ?>
<?php include_once $EW_RELATIVE_PATH . "municipiomaster.php" ?>
<?php
	}
}
?>
...
<?php
	$bSelectLimit = EW_SELECT_LIMIT;
	if ($bSelectLimit) {
		$hospital_list->TotalRecs = $hospital->SelectRecordCount();
	} else {
		if ($hospital_list->Recordset = $hospital_list->LoadRecordset())
			$hospital_list->TotalRecs = $hospital_list->Recordset->RecordCount();
	}
	$hospital_list->StartRec = 1;
	if ($hospital_list->DisplayRecs <= 0 || ($hospital->Export <> "" && $hospital->ExportAll)) // Display all records
		$hospital_list->DisplayRecs = $hospital_list->TotalRecs;
	if (!($hospital->Export <> "" && $hospital->ExportAll))
		$hospital_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$hospital_list->Recordset = $hospital_list->LoadRecordset($hospital_list->StartRec-1, $hospital_list->DisplayRecs);

	// Set no record found message
	if ($hospital->CurrentAction == "" && $hospital_list->TotalRecs == 0) {
		if ($hospital_list->SearchWhere == "0=101")
			$hospital_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$hospital_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$hospital_list->RenderOtherOptions();
?>
<?php if ($hospital->Export == "" && $hospital->CurrentAction == "") { ?>
<form name="fhospitallistsrch" id="fhospitallistsrch" class="form-inline ewForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($hospital_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fhospitallistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="hospital">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($hospital_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($hospital_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $hospital_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($hospital_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($hospital_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($hospital_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($hospital_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<div class="ewGrid">
<form name="fhospitallist" id="fhospitallist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($hospital_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $hospital_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="hospital">
<div id="gmp_hospital" class="<?php if (ew_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php if ($hospital_list->TotalRecs > 0) { ?>
<table id="tbl_hospitallist" class="table ewTable">
...
<?php if ($hospital->nombre->Visible) { // nombre ?>
	...
	<?php } else { ?>
		<th data-name="nombre"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $hospital->SortUrl($hospital->nombre) ?>',1);"><div id="elh_hospital_nombre" class="hospital_nombre">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $hospital->nombre->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($hospital->nombre->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($hospital->nombre->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($hospital->telefono->Visible) { // telefono ?>
	...
	<?php } else { ?>
		<th data-name="telefono"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $hospital->SortUrl($hospital->telefono) ?>',1);"><div id="elh_hospital_telefono" class="hospital_telefono">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $hospital->telefono->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($hospital->telefono->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($hospital->telefono->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($hospital->estado->Visible) { // estado ?>
	...
	<?php } else { ?>
		<th data-name="estado"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $hospital->SortUrl($hospital->estado) ?>',1);"><div id="elh_hospital_estado" class="hospital_estado">
...
<tbody>
<?php
if ($hospital->ExportAll && $hospital->Export <> "") {
...
if ($hospital_list->Recordset && !$hospital_list->Recordset->EOF) {
	...
	$bSelectLimit = EW_SELECT_LIMIT;
	if (!$bSelectLimit && $hospital_list->StartRec > 1)
...
while ($hospital_list->RecCnt < $hospital_list->StopRec) {
	...
	if (intval($hospital_list->RecCnt) >= intval($hospital_list->StartRec)) {
...
<a id="<?php echo $hospital_list->PageObjName . "_row_" . $hospital_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($hospital->telefono->Visible) { // telefono ?>
...
<?php if ($hospital->CurrentAction <> "gridadd" && $hospital->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($hospital_list->Pager)) $hospital_list->Pager = new cPrevNextPager($hospital_list->StartRec, $hospital_list->DisplayRecs, $hospital_list->TotalRecs) ?>
...
<!--first page button-->
	...
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $hospital_list->PageUrl() ?>start=<?php echo $hospital_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($hospital_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $hospital_list->PageUrl() ?>start=<?php echo $hospital_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $hospital_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($hospital_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $hospital_list->PageUrl() ?>start=<?php echo $hospital_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($hospital_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $hospital_list->PageUrl() ?>start=<?php echo $hospital_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $hospital_list->Pager->PageCount ?></span>
</div>
...
<?php
	foreach ($hospital_list->OtherOptions as &$option)
