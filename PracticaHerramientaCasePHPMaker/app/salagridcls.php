...
$sala_grid = NULL; // Initialize page object first

class csala_grid extends csala {
	...
	// Page object name
	var $PageObjName = 'sala_grid';

	// Grid form hidden field names
	var $FormName = 'fsalagrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["sala"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "nivel") {
			global $nivel;
			$rsmaster = $nivel->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("nivellist.php"); // Return to master page
			} else {
				$nivel->LoadListRowValues($rsmaster);
				$nivel->RowType = EW_ROWTYPE_MASTER; // Master row
				$nivel->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->idsala->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->idsala->FormValue))
				return FALSE;
		}
		return TRUE;
	}
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_estado") && $objForm->HasValue("o_estado") && $this->estado->CurrentValue <> $this->estado->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_idnivel") && $objForm->HasValue("o_idnivel") && $this->idnivel->CurrentValue <> $this->idnivel->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->idsala->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->idnivel->setOldValue($objForm->GetValue("o_idnivel"));
		if (!$this->idsala->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idsala->CurrentValue = strval($arKeys[0]); // idsala
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "nivel") {
			...
			if ($GLOBALS["nivel"]->EventCancelled) $this->EventCancelled = TRUE;
		}
