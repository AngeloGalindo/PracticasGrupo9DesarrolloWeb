...
$departamento_view = NULL; // Initialize page object first

class cdepartamento_view extends cdepartamento {
	...
	function __construct() {
		...
		if (@$_GET["iddepartamento"] <> "") {
			$this->RecKey["iddepartamento"] = $_GET["iddepartamento"];
			$KeyUrl .= "&amp;iddepartamento=" . urlencode($this->RecKey["iddepartamento"]);
		}
	...
	function Page_Init() {
		...
		$this->iddepartamento->Visible = !$this->IsAdd() && !$this->IsCopy() && !$this->IsGridAdd();
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["iddepartamento"] <> "") {
				...
				$this->RecKey["iddepartamento"] = $this->iddepartamento->QueryStringValue;
			} else {
	...
	function SetupOtherOptions() {
		...
		// "detail_municipio"
		$item = &$option->Add("detail_municipio");
		...
		$links = "";
		if ($GLOBALS["municipio_grid"] && $GLOBALS["municipio_grid"]->DetailView) {
		...
		if ($GLOBALS["municipio_grid"] && $GLOBALS["municipio_grid"]->DetailEdit) {
		...
		if ($GLOBALS["municipio_grid"] && $GLOBALS["municipio_grid"]->DetailAdd) {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("municipio", $DetailTblVar)) {
				...
				if ($GLOBALS["municipio_grid"]->DetailView) {
					$GLOBALS["municipio_grid"]->CurrentMode = "view";
...
// Create page object
if (!isset($departamento_view)) $departamento_view = new cdepartamento_view();
...
// Form object
var fdepartamentoview = new ew_Form("fdepartamentoview");

// Form_CustomValidate event
fdepartamentoview.Form_CustomValidate = 
...
fdepartamentoview.ValidateRequired = true;
<?php } else { ?>
fdepartamentoview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fdepartamentoview.Lists["x_idpais"] = {"LinkField":"x_idpais","Ajax":true,"AutoFill":false,"DisplayFields":["x_nombre","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $departamento_view->ExportOptions->Render("body") ?>
<?php
	foreach ($departamento_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php echo $Language->SelectionForm(); ?>
<div class="clearfix"></div>
</div>
<?php $departamento_view->ShowPageHeader(); ?>
<?php
$departamento_view->ShowMessage();
?>
<form name="fdepartamentoview" id="fdepartamentoview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($departamento_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $departamento_view->Token ?>">
<?php } ?>
...
<?php if ($departamento->iddepartamento->Visible) { // iddepartamento ?>
	<tr id="r_iddepartamento">
		<td><span id="elh_departamento_iddepartamento"><?php echo $departamento->iddepartamento->FldCaption() ?></span></td>
		<td<?php echo $departamento->iddepartamento->CellAttributes() ?>>
<span id="el_departamento_iddepartamento" class="form-group">
<span<?php echo $departamento->iddepartamento->ViewAttributes() ?>>
<?php echo $departamento->iddepartamento->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($departamento->nombre->Visible) { // nombre ?>
	<tr id="r_nombre">
		<td><span id="elh_departamento_nombre"><?php echo $departamento->nombre->FldCaption() ?></span></td>
...
<?php echo $departamento->nombre->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($departamento->idpais->Visible) { // idpais ?>
	<tr id="r_idpais">
		<td><span id="elh_departamento_idpais"><?php echo $departamento->idpais->FldCaption() ?></span></td>
...
<?php echo $departamento->idpais->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php
	if (in_array("municipio", explode(",", $departamento->getCurrentDetailTable())) && $municipio->DetailView) {
?>
...
<script type="text/javascript">
fdepartamentoview.Init();
</script>
<?php
$departamento_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$departamento_view->Page_Terminate();
?>
