...
$medicina_grid = NULL; // Initialize page object first

class cmedicina_grid extends cmedicina {
	...
	// Page object name
	var $PageObjName = 'medicina_grid';

	// Grid form hidden field names
	var $FormName = 'fmedicinagrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["medicina"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->idmedicina->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->idmedicina->FormValue))
				return FALSE;
		}
		return TRUE;
	}
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->idmedicina->CurrentValue;
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_estado") && $objForm->HasValue("o_estado") && $this->estado->CurrentValue <> $this->estado->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_idlaboratorio") && $objForm->HasValue("o_idlaboratorio") && $this->idlaboratorio->CurrentValue <> $this->idlaboratorio->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->idmedicina->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		$this->idmedicina->CurrentValue = NULL;
	...
	function LoadFormValues() {
		...
		if (!$this->idmedicina->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
		...
		$this->idlaboratorio->setOldValue($objForm->GetValue("o_idlaboratorio"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idmedicina->CurrentValue = strval($arKeys[0]); // idmedicina
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "laboratorio") {
			...
			if ($GLOBALS["laboratorio"]->EventCancelled) $this->EventCancelled = TRUE;
		}
