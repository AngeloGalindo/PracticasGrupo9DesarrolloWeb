...
$nivel_grid = NULL; // Initialize page object first

class cnivel_grid extends cnivel {
	...
	// Page object name
	var $PageObjName = 'nivel_grid';

	// Grid form hidden field names
	var $FormName = 'fnivelgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["nivel"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->idnivel->CurrentValue;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->idnivel->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		if (!$this->idnivel->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idnivel->CurrentValue = strval($arKeys[0]); // idnivel
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
