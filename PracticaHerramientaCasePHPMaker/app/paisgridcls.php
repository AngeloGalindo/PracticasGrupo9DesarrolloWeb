...
$pais_grid = NULL; // Initialize page object first

class cpais_grid extends cpais {
	...
	// Page object name
	var $PageObjName = 'pais_grid';

	// Grid form hidden field names
	var $FormName = 'fpaisgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["pais"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_nombre") && $objForm->HasValue("o_nombre") && $this->nombre->CurrentValue <> $this->nombre->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nombre_oficial") && $objForm->HasValue("o_nombre_oficial") && $this->nombre_oficial->CurrentValue <> $this->nombre_oficial->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_gentilicio") && $objForm->HasValue("o_gentilicio") && $this->gentilicio->CurrentValue <> $this->gentilicio->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_flag") && $objForm->HasValue("o_flag") && $this->flag->CurrentValue <> $this->flag->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_idcontinente") && $objForm->HasValue("o_idcontinente") && $this->idcontinente->CurrentValue <> $this->idcontinente->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->idpais->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		...
		$this->nombre_oficial->CurrentValue = NULL;
		$this->nombre_oficial->OldValue = $this->nombre_oficial->CurrentValue;
		$this->gentilicio->CurrentValue = NULL;
		$this->gentilicio->OldValue = $this->gentilicio->CurrentValue;
		$this->flag->CurrentValue = NULL;
		$this->flag->OldValue = $this->flag->CurrentValue;
		$this->idcontinente->CurrentValue = 1;
	...
	function LoadFormValues() {
		...
		$this->nombre_oficial->setOldValue($objForm->GetValue("o_nombre_oficial"));
		if (!$this->gentilicio->FldIsDetailKey) {
			$this->gentilicio->setFormValue($objForm->GetValue("x_gentilicio"));
		}
		$this->gentilicio->setOldValue($objForm->GetValue("o_gentilicio"));
		...
		$this->idcontinente->setOldValue($objForm->GetValue("o_idcontinente"));
		if (!$this->estado->FldIsDetailKey) {
			$this->estado->setFormValue($objForm->GetValue("x_estado"));
		}
		$this->estado->setOldValue($objForm->GetValue("o_estado"));
		if (!$this->idpais->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idpais->CurrentValue = strval($arKeys[0]); // idpais
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		// Set up foreign key field value from Session
			if ($this->getCurrentMasterTable() == "continente") {
				$this->idcontinente->CurrentValue = $this->idcontinente->getSessionValue();
			}
		...
		// nombre oficial
		$this->nombre_oficial->SetDbValueDef($rsnew, $this->nombre_oficial->CurrentValue, NULL, FALSE);

		// gentilicio
		$this->gentilicio->SetDbValueDef($rsnew, $this->gentilicio->CurrentValue, NULL, FALSE);

		// flag
		$this->flag->SetDbValueDef($rsnew, $this->flag->CurrentValue, NULL, FALSE);

		// idcontinente
		$this->idcontinente->SetDbValueDef($rsnew, $this->idcontinente->CurrentValue, 0, strval($this->idcontinente->CurrentValue) == "");
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "continente") {
			$this->idcontinente->Visible = FALSE;
			if ($GLOBALS["continente"]->EventCancelled) $this->EventCancelled = TRUE;
		}
