...
$habitacion_view = NULL; // Initialize page object first

class chabitacion_view extends chabitacion {
	...
	// Page object name
	var $PageObjName = 'habitacion_view';
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["idhabitacion"] <> "") {
			$this->RecKey["idhabitacion"] = $_GET["idhabitacion"];
			$KeyUrl .= "&amp;idhabitacion=" . urlencode($this->RecKey["idhabitacion"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["idhabitacion"] <> "") {
				$this->idhabitacion->setQueryStringValue($_GET["idhabitacion"]);
				$this->RecKey["idhabitacion"] = $this->idhabitacion->QueryStringValue;
			} else {
				$sReturnUrl = "habitacionlist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "habitacionlist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "habitacionlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("internado", $DetailTblVar)) {
				...
				if ($GLOBALS["internado_grid"]->DetailView) {
					...
					$GLOBALS["internado_grid"]->idhabitacion->FldIsDetailKey = TRUE;
					$GLOBALS["internado_grid"]->idhabitacion->CurrentValue = $this->idhabitacion->CurrentValue;
					$GLOBALS["internado_grid"]->idhabitacion->setSessionValue($GLOBALS["internado_grid"]->idhabitacion->CurrentValue);
				}
			}
		}
	}
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", $this->TableVar, "habitacionlist.php", "", $this->TableVar, TRUE);
		$PageId = "view";
...
// Create page object
if (!isset($habitacion_view)) $habitacion_view = new chabitacion_view();

// Page init
$habitacion_view->Page_Init();

// Page main
$habitacion_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$habitacion_view->Page_Render();
?>
<?php include_once $EW_RELATIVE_PATH . "header.php" ?>
<script type="text/javascript">

// Page object
var habitacion_view = new ew_Page("habitacion_view");
habitacion_view.PageID = "view"; // Page ID
var EW_PAGE_ID = habitacion_view.PageID; // For backward compatibility

// Form object
var fhabitacionview = new ew_Form("fhabitacionview");

// Form_CustomValidate event
fhabitacionview.Form_CustomValidate = 
...
fhabitacionview.ValidateRequired = true;
<?php } else { ?>
fhabitacionview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fhabitacionview.Lists["x_idsala"] = {"LinkField":"x_idsala","Ajax":true,"AutoFill":false,"DisplayFields":["x_descripcion","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $habitacion_view->ExportOptions->Render("body") ?>
<?php
	foreach ($habitacion_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php echo $Language->SelectionForm(); ?>
<div class="clearfix"></div>
</div>
<?php $habitacion_view->ShowPageHeader(); ?>
<?php
$habitacion_view->ShowMessage();
?>
<form name="fhabitacionview" id="fhabitacionview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($habitacion_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $habitacion_view->Token ?>">
<?php } ?>
...
<?php if ($habitacion->idhabitacion->Visible) { // idhabitacion ?>
	<tr id="r_idhabitacion">
		<td><span id="elh_habitacion_idhabitacion"><?php echo $habitacion->idhabitacion->FldCaption() ?></span></td>
...
<?php echo $habitacion->idhabitacion->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($habitacion->numero->Visible) { // numero ?>
	<tr id="r_numero">
		<td><span id="elh_habitacion_numero"><?php echo $habitacion->numero->FldCaption() ?></span></td>
...
<?php echo $habitacion->numero->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($habitacion->idsala->Visible) { // idsala ?>
	<tr id="r_idsala">
		<td><span id="elh_habitacion_idsala"><?php echo $habitacion->idsala->FldCaption() ?></span></td>
...
<?php echo $habitacion->idsala->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($habitacion->estado->Visible) { // estado ?>
	<tr id="r_estado">
		<td><span id="elh_habitacion_estado"><?php echo $habitacion->estado->FldCaption() ?></span></td>
...
<?php echo $habitacion->estado->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php
	if (in_array("internado", explode(",", $habitacion->getCurrentDetailTable())) && $internado->DetailView) {
?>
<?php if ($habitacion->getCurrentDetailTable() <> "") { ?>
...
<script type="text/javascript">
fhabitacionview.Init();
</script>
<?php
$habitacion_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$habitacion_view->Page_Terminate();
?>
