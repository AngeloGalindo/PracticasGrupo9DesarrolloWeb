...
$servicio_medico_prestado_grid = NULL; // Initialize page object first

class cservicio_medico_prestado_grid extends cservicio_medico_prestado {
	...
	// Page object name
	var $PageObjName = 'servicio_medico_prestado_grid';

	// Grid form hidden field names
	var $FormName = 'fservicio_medico_prestadogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["servicio_medico_prestado"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->idservicio_medico_prestado->CurrentValue;
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_idcuenta") && $objForm->HasValue("o_idcuenta") && $this->idcuenta->CurrentValue <> $this->idcuenta->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_idservicio_medico") && $objForm->HasValue("o_idservicio_medico") && $this->idservicio_medico->CurrentValue <> $this->idservicio_medico->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_estado") && $objForm->HasValue("o_estado") && $this->estado->CurrentValue <> $this->estado->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_costo") && $objForm->HasValue("o_costo") && $this->costo->CurrentValue <> $this->costo->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_fecha_inicio") && $objForm->HasValue("o_fecha_inicio") && $this->fecha_inicio->CurrentValue <> $this->fecha_inicio->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_fecha_final") && $objForm->HasValue("o_fecha_final") && $this->fecha_final->CurrentValue <> $this->fecha_final->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->idservicio_medico_prestado->CurrentValue . "\">";
		}
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('idservicio_medico_prestado');
	}
	...
	function LoadDefaultValues() {
		...
		$this->idservicio_medico->OldValue = $this->idservicio_medico->CurrentValue;
		$this->estado->CurrentValue = "Solicitado";
		...
		$this->fecha_final->CurrentValue = NULL;
		$this->fecha_final->OldValue = $this->fecha_final->CurrentValue;
	}
	...
	function LoadFormValues() {
		...
		$this->idservicio_medico->setOldValue($objForm->GetValue("o_idservicio_medico"));
		...
		$this->costo->setOldValue($objForm->GetValue("o_costo"));
		...
		if (!$this->fecha_final->FldIsDetailKey) {
			$this->fecha_final->setFormValue($objForm->GetValue("x_fecha_final"));
			$this->fecha_final->CurrentValue = ew_UnFormatDateTime($this->fecha_final->CurrentValue, 7);
		}
		$this->fecha_final->setOldValue($objForm->GetValue("o_fecha_final"));
		if (!$this->idservicio_medico_prestado->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function RestoreFormValues() {
		...
		$this->fecha_inicio->CurrentValue = ew_UnFormatDateTime($this->fecha_inicio->CurrentValue, 7);
		$this->fecha_final->CurrentValue = $this->fecha_final->FormValue;
		$this->fecha_final->CurrentValue = ew_UnFormatDateTime($this->fecha_final->CurrentValue, 7);
	}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idservicio_medico_prestado->CurrentValue = strval($arKeys[0]); // idservicio_medico_prestado
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$this->costo->OldValue = $this->costo->EditValue;
			}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// fecha_final
			$this->fecha_final->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->fecha_final->CurrentValue, 7), NULL, $this->fecha_final->ReadOnly);
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "cuenta") {
			...
			if ($GLOBALS["cuenta"]->EventCancelled) $this->EventCancelled = TRUE;
		}
		if ($sMasterTblVar == "servicio_medico") {
			$this->idservicio_medico->Visible = FALSE;
			if ($GLOBALS["servicio_medico"]->EventCancelled) $this->EventCancelled = TRUE;
		}
