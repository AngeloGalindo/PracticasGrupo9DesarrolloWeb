...
$internado_diario_grid = NULL; // Initialize page object first

class cinternado_diario_grid extends cinternado_diario {
	...
	// Page object name
	var $PageObjName = 'internado_diario_grid';

	// Grid form hidden field names
	var $FormName = 'finternado_diariogrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["internado_diario"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->idinternado_diario->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_idinternado") && $objForm->HasValue("o_idinternado") && $this->idinternado->CurrentValue <> $this->idinternado->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->idinternado_diario->CurrentValue . "\">";
		}
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('idinternado_diario');
	}
	...
	function LoadDefaultValues() {
		$this->idinternado_diario->CurrentValue = NULL;
		$this->idinternado_diario->OldValue = $this->idinternado_diario->CurrentValue;
	...
	function LoadFormValues() {
		...
		if (!$this->idinternado_diario->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
		...
		$this->idinternado->setOldValue($objForm->GetValue("o_idinternado"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idinternado_diario->CurrentValue = strval($arKeys[0]); // idinternado_diario
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "internado") {
			$this->idinternado->Visible = FALSE;
			if ($GLOBALS["internado"]->EventCancelled) $this->EventCancelled = TRUE;
		}
