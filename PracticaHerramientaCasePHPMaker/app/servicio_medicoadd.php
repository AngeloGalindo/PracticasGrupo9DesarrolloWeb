...
$servicio_medico_add = NULL; // Initialize page object first

class cservicio_medico_add extends cservicio_medico {
	...
	// Page object name
	var $PageObjName = 'servicio_medico_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["idservicio_medico"] != "") {
				...
				$this->setKey("idservicio_medico", $this->idservicio_medico->CurrentValue); // Set up key
			} else {
				$this->setKey("idservicio_medico", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "servicio_medicoview.php")
	...
	function LoadOldRecord() {
		...
		$bValidKey = TRUE;
		if (strval($this->getKey("idservicio_medico")) <> "")
			$this->idservicio_medico->CurrentValue = $this->getKey("idservicio_medico"); // idservicio_medico
		else
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			$this->idservicio_medico->setDbValue($conn->Insert_ID());
			$rsnew['idservicio_medico'] = $this->idservicio_medico->DbValue;
		}
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("servicio_medico_prestado", $DetailTblVar) && $GLOBALS["servicio_medico_prestado"]->DetailAdd) {
				$GLOBALS["servicio_medico_prestado"]->idservicio_medico->setSessionValue($this->idservicio_medico->CurrentValue); // Set master key
				if (!isset($GLOBALS["servicio_medico_prestado_grid"])) $GLOBALS["servicio_medico_prestado_grid"] = new cservicio_medico_prestado_grid(); // Get detail page object
				$AddRow = $GLOBALS["servicio_medico_prestado_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["servicio_medico_prestado"]->idservicio_medico->setSessionValue(""); // Clear master key if insert failed
			}
		}
...
// Create page object
if (!isset($servicio_medico_add)) $servicio_medico_add = new cservicio_medico_add();

// Page init
$servicio_medico_add->Page_Init();

// Page main
$servicio_medico_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$servicio_medico_add->Page_Render();
?>
<?php include_once $EW_RELATIVE_PATH . "header.php" ?>
<script type="text/javascript">

// Page object
var servicio_medico_add = new ew_Page("servicio_medico_add");
servicio_medico_add.PageID = "add"; // Page ID
var EW_PAGE_ID = servicio_medico_add.PageID; // For backward compatibility

// Form object
var fservicio_medicoadd = new ew_Form("fservicio_medicoadd");

// Validate form
fservicio_medicoadd.Validate = function() {
...
fservicio_medicoadd.Form_CustomValidate = 
...
fservicio_medicoadd.ValidateRequired = true;
<?php } else { ?>
fservicio_medicoadd.ValidateRequired = false; 
<?php } ?>
...
<?php $servicio_medico_add->ShowPageHeader(); ?>
<?php
$servicio_medico_add->ShowMessage();
?>
<form name="fservicio_medicoadd" id="fservicio_medicoadd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($servicio_medico_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $servicio_medico_add->Token ?>">
<?php } ?>
...
<?php
	if (in_array("servicio_medico_prestado", explode(",", $servicio_medico->getCurrentDetailTable())) && $servicio_medico_prestado->DetailAdd) {
?>
...
<script type="text/javascript">
fservicio_medicoadd.Init();
</script>
<?php
$servicio_medico_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$servicio_medico_add->Page_Terminate();
?>
