...
$servicio_medico_prestado = NULL;

//
// Table class for servicio_medico_prestado
//
class cservicio_medico_prestado extends cTable {
	...
	function GetMasterFilter() {
		...
		if ($this->getCurrentMasterTable() == "servicio_medico") {
			if ($this->idservicio_medico->getSessionValue() <> "")
				$sMasterFilter .= "`idservicio_medico`=" . ew_QuotedValue($this->idservicio_medico->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "servicio_medico") {
			if ($this->idservicio_medico->getSessionValue() <> "")
				$sDetailFilter .= "`idservicio_medico`=" . ew_QuotedValue($this->idservicio_medico->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
	...
	// Master filter
	function SqlMasterFilter_servicio_medico() {
		return "`idservicio_medico`=@idservicio_medico@";
	}

	// Detail filter
	function SqlDetailFilter_servicio_medico() {
		return "`idservicio_medico`=@idservicio_medico@";
	}
	...
	function GetDetailUrl() {
		...
		// Detail url
		$sDetailUrl = "";
		if ($this->getCurrentDetailTable() == "doctor_servicio_medico_prestado") {
			$sDetailUrl = $GLOBALS["doctor_servicio_medico_prestado"]->GetListUrl() . "?showmaster=" . $this->TableVar;
			$sDetailUrl .= "&fk_idservicio_medico_prestado=" . urlencode($this->idservicio_medico_prestado->CurrentValue);
		}
		if ($sDetailUrl == "") {
			$sDetailUrl = "servicio_medico_prestadolist.php";
		}
		return $sDetailUrl;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`servicio_medico_prestado`";
	}
	...
	// Update Table
	var $UpdateTable = "`servicio_medico_prestado`";
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('idservicio_medico_prestado', $rs))
				ew_AddFilter($where, ew_QuotedName('idservicio_medico_prestado') . '=' . ew_QuotedValue($rs['idservicio_medico_prestado'], $this->idservicio_medico_prestado->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->idservicio_medico_prestado->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@idservicio_medico_prestado@", ew_AdjustSql($this->idservicio_medico_prestado->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "servicio_medico_prestadolist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("servicio_medico_prestadoview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("servicio_medico_prestadoview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "servicio_medico_prestadoadd.php?" . $this->UrlParm($parm);
		else
			return "servicio_medico_prestadoadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("servicio_medico_prestadoedit.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("servicio_medico_prestadoedit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("servicio_medico_prestadoadd.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("servicio_medico_prestadoadd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("servicio_medico_prestadodelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->idservicio_medico_prestado->CurrentValue)) {
			$sUrl .= "idservicio_medico_prestado=" . urlencode($this->idservicio_medico_prestado->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["idservicio_medico_prestado"]; // idservicio_medico_prestado
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->idservicio_medico_prestado->CurrentValue = $key;
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->idservicio_medico_prestado->EditValue = $this->idservicio_medico_prestado->CurrentValue;
		$this->idservicio_medico_prestado->ViewCustomAttributes = "";
